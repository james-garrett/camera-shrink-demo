...
func _ready() -> void:
	...
	configCamera()
	if !loggedPlayerReadyState && CustomLogger.isReady():
		CustomLogger.log("Player Ready")
		loggedPlayerReadyState = true
...
func configCamera() -> void: 
	...
	ProxyCamera = root_scene.find_child("ProxyCamera", true, false)
	await get_tree().process_frame
	Camera.current = false
	ProxyCamera.current = true
...
func _process(_delta: float) -> void:
	...
	if cameraConfigured:
		ProxyCamera.global_transform = Camera.global_transform
		
	if Camera.current == true:
		print("Wait it's true now what??")
